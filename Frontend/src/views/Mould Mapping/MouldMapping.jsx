...
//     console.log("🚀 ~ FGMouldMapping ~ pageData:", pageData)
//     if (!pageData) {
...
const MouldMapping = ({ auth }) => {
    ...
    if (permission) {
        localStorage.setItem('mouldMappingPermission', JSON.stringify(permission));
    } else {
        const stored = localStorage.getItem('mouldMappingPermission');
    ...
    const [MouldPartData, setMouldPartData] = useState([]);
    const [MappingData, setMappingData] = useState([]);
    const [selectedMaster, setSelectedMaster] = useState([]);
    const [selectedMouldParts, setSelectedMouldParts] = useState([]);
    const [expanded, setExpanded] = useState({});
    const [searchSelection, setSearchSelection] = useState([]);

    const emptyPayload = { MapID: '', M_ID: '', MP_ID: '', CreatedBy: '' };

    // ---------------- Fetchers ----------------
    const fetchMaster = async () => {
        try {
            const response = await axios.post(`${API_URL}/MouldMappingConfig`, { ...emptyPayload, mode: 'GetMaster' });
            setMasterData(response.data);
        } catch (err) { console.error('Error fetching Master list:', err); }
    };
    const fetchMouldPart = async () => {
        try {
            const response = await axios.post(`${API_URL}/MouldMappingConfig`, { ...emptyPayload, mode: 'GetMouldPart' });
            setMouldPartData(response.data);
        } catch (err) { console.error('Error fetching Mould Part list:', err); }
    };
    const fetchMapping = async () => {
        setLoading(true);
        try {
            const response = await axios.post(`${API_URL}/MouldMappingConfig`, {
                ...emptyPayload,
                mode: 'FetchMapping',
    ...
    // Only mould parts not yet mapped anywhere are selectable
    const freeMouldParts = useMemo(
        () => MouldPartData.filter(m => !m.MappedMasterCode),
        [MouldPartData]
    );

    // ---------------- Group mappings Master-wise ----------------
    const allGroups = useMemo(() => {
        const groups = {};
        MappingData.forEach(row => {
            if (!groups[row.M_ID]) {
            ...
            groups[row.M_ID].mouldParts.push(row);
        });
        return Object.values(groups).sort((a, b) => a.MasterCode.localeCompare(b.MasterCode));
    }, [MappingData]);

    // What actually renders: filtered by the search Typeahead
    const masterGroups = useMemo(() => {
        if (searchSelection.length > 0) {
            return allGroups.filter(g => g.M_ID === searchSelection[0].M_ID);
        }
        return allGroups;
    }, [allGroups, searchSelection]);
    ...
    const handleMap = async () => {
        if (selectedMaster.length === 0) {
            Swal.fire({ title: 'Please Select Item Code', icon: 'warning', confirmButtonText: 'Done' });
            return;
        }
        if (selectedMouldParts.length === 0) {
            Swal.fire({ title: 'Please Select at least One Mould Item Code', icon: 'warning', confirmButtonText: 'Done' });
            return;
        }
        setLoading(true);
        try {
            const alldata = {
                ...emptyPayload,
                mode: 'I',
                M_ID: selectedMaster[0].Id,
                MP_IDs: selectedMouldParts.map(m => m.MP_ID),
                CreatedBy: auth.employeename,
                branchid: auth.branchid
            };
            const response = await axios.post(`${API_URL}/MouldMappingConfig`, alldata);
            const { mappedCount, skipped } = response.data;

            if (skipped && skipped.length > 0) {
                const skippedText = skipped.map(s => `MP_ID ${s.MP_ID} → already in ${s.MappedTo}`).join('\n');
                swal({
                    title: `Mapped: ${mappedCount}, Skipped: ${skipped.length}`,
                    text: `Some mould parts were already mapped:\n${skippedText}`,
                    icon: 'warning'
                });
            } else {
                Swal.fire({
                    title: 'Mapped Successfully',
                    text: `${mappedCount} Mould Part(s) mapped to ${selectedMaster[0].ItemCode}`,
                    icon: 'success',
                    confirmButtonText: 'Done'
                });
            }
            setSelectedMouldParts([]);
            setSelectedMaster([]);
            fetchMouldPart();
            fetchMapping();
        } catch (err) {
    ...
    const handleUnmap = async (row) => {
        const result = await Swal.fire({
            title: 'Are you sure?',
            text: `Unmap Mould ${row.MouldPartCode} (${row.MouldPart}) from ${row.MasterCode}?`,
            icon: 'warning',
            showCancelButton: true,
            cancelButtonText: 'Cancel',
            confirmButtonText: 'Unmap',
    ...
    const downloadCSV = (rows, fileName) => {
        ...
        const headers = ['Item Code', 'Wheel Size', 'Module Size', 'Grade', 'Mould Item Code', 'Mould Part', 'Short Form Size', 'RFID', 'Item Description', 'Mapped By', 'Mapped Date'];
        const csvRows = rows.map(r => [
            r.MasterCode, r.WheelSize, r.ModuleSize, r.Grade,
            r.MouldPartCode, r.MouldPart, r.ShortFormSize, r.RFID,
            (r.ItemDescription || '').replace(/,/g, ' '),
            r.CreatedBy || '',
            r.CreatedDate ? new Date(r.CreatedDate).toISOString().slice(0, 19).replace('T', ' ') : ''
        ].map(v => `"${v ?? ''}"`).join(','));
    ...
    const exportAllCSV = () => downloadCSV(MappingData, 'Mould_Mapping.csv');
    const exportMasterCSV = (group) => downloadCSV(group.mouldParts, `Mould_${group.MasterCode}_Parts.csv`);
    ...
    const generatePDF = () => {
        ...
        try {
            ...
            const columnMapping = [
                { header: 'Item Code', key: 'MasterCode' },
                { header: 'Wheel Size', key: 'WheelSize' },
                { header: 'Module Size', key: 'ModuleSize' },
                { header: 'Grade', key: 'Grade' },
                { header: 'Mould Item Code', key: 'MouldPartCode' },
                { header: 'Mould Part', key: 'MouldPart' },
                { header: 'Short Form Size', key: 'ShortFormSize' },
                { header: 'RFID', key: 'RFID' },
                { header: 'Mapped By', key: 'CreatedBy' },
    ...
    return (
        <>
            ...
            <div className="mt-3">
                ...
                <div className="card shadow-sm border-0 mb-3" style={{ borderRadius: '14px' }}>
                    ...
                    {canAdd && (
                        <div className="px-4 py-3"
                            ...
                            <div className="row align-items-end">
                                <div className="col-lg-3 col-md-6 mt-2">
                                    <label className="fw-semibold">Item Code <span style={{ color: 'red' }}>*</span></label>
                                    <Typeahead
                                        id="master-typeahead"
                                        positionFixed
                                        labelKey={(option) => `${option.ItemCode}`}
                                        onChange={(selected) => setSelectedMaster(selected)}
                                        options={MasterData}
                                ...
                                <div className="col-lg-2 col-md-6 mt-2">
                                    <label className="fw-semibold">Wheel Size</label>
                                    <input className="form-control" value={selectedMaster[0]?.WheelSize || ''} disabled />
                                </div>
                                <div className="col-lg-4 col-md-8 mt-2">
                                    <label className="fw-semibold">Mould Item Code(s) <span style={{ color: 'red' }}>*</span></label>
                                    <Typeahead
                                        id="mould-part-typeahead"
                                        multiple
                                        positionFixed
                                        labelKey={(option) => `${option.ItemCode} (${option.MouldPart})`}
                                        onChange={(selected) => setSelectedMouldParts(selected)}
                                        options={freeMouldParts}
                                        placeholder={freeMouldParts.length === 0 ? 'No Unmapped Mould Parts Available' : 'Select Mould Item Code(s)'}
                                        selected={selectedMouldParts}
                                        clearButton
                                        renderMenuItemChildren={(option) => (
                                            <div>
                                                <strong>{option.ItemCode}</strong> — {option.MouldPart}
                ...
                {/* ======= Master-wise Mapping List ======= */}
                <div className="card shadow-sm border-0" style={{ borderRadius: '14px' }}>
                    <div className="card-body">
                        <div className="d-flex justify-content-between align-items-center flex-wrap mb-3">
                            <h5 className="mb-0" style={{ color: '#2e3f7e' }}>
                                <i className="bi bi-list-nested me-2"></i>Item Wise Mould Mapping
                                <span className="badge rounded-pill ms-2" style={{ background: '#3f77d2' }}>
                                    {masterGroups.length} Item{masterGroups.length !== 1 ? 's' : ''}
                                </span>
                            </h5>

                            {/* Search Typeahead: matches Item Code, Grade, Mould Item Code, Mould Part, RFID */}
                            <div style={{ minWidth: '320px' }}>
                                <Typeahead
                                    id="mapping-search-typeahead"
                                    positionFixed
                                    ...
                                    labelKey="MasterCode"
                                    options={allGroups}
                                    placeholder="Search Item / Mould / Grade..."
                                    selected={searchSelection}
                                    onChange={(selected) => {
                                        setSearchSelection(selected);
                                        if (selected.length > 0) {
                                            setExpanded(prev => ({ ...prev, [selected[0].M_ID]: true }));
                                        }
                                    }}
                                    filterBy={(option, props) => {
                                        const s = props.text.toLowerCase();
                                        return option.MasterCode.toLowerCase().includes(s) ||
                                            (option.Grade || '').toLowerCase().includes(s) ||
                                            option.mouldParts.some(m =>
                                                m.MouldPartCode.toLowerCase().includes(s) ||
                                                (m.MouldPart || '').toLowerCase().includes(s) ||
                                                (m.RFID || '').toLowerCase().includes(s)
                                            );
                                    }}
                                    renderMenuItemChildren={(option) => (
                                        <div>
                                            <strong>{option.MasterCode}</strong>
                                            <div className="small text-muted">
                                                Grade: {option.Grade} | {option.mouldParts.length} Mould{option.mouldParts.length > 1 ? 's' : ''}
                                            </div>
                                        </div>
                                    )}
                                />
                        ...
                        {masterGroups.length === 0 && (
                            <div className="text-center text-muted py-5">
                                <i className="bi bi-inboxes fs-1 d-block mb-2"></i>
                                No Mould mappings found. Select an Item Code and Mould(s) above to map.
                            </div>
                        )}

                        {masterGroups.map((group) => (
                            <div key={group.M_ID} className="mb-3 border rounded-3 overflow-hidden shadow-sm">
                                {/* Master Header Row */}
                                <div
                                    className="d-flex justify-content-between align-items-center px-3 py-2"
                                    style={{ background: '#eef4ff', cursor: 'pointer' }}
                                    onClick={() => toggleExpand(group.M_ID)}
                                >
                                    <div className="d-flex align-items-center flex-wrap gap-3">
                                        <i className={`bi ${expanded[group.M_ID] ? 'bi-chevron-down' : 'bi-chevron-right'} fs-5`}
                                            style={{ color: '#3f77d2', transition: 'transform 0.2s' }}></i>
                                        <div>
                                            <span className="fw-bold" style={{ color: '#2e3f7e', fontSize: '1.05rem' }}>
                                                {group.MasterCode}
                                            ...
                                            <div className="small text-muted">
                                                Wheel: {group.WheelSize} &nbsp;|&nbsp; Module Size: {group.ModuleSize} &nbsp;|&nbsp; Grade: {group.Grade}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="d-flex align-items-center gap-2">
                                        <span className="badge rounded-pill"
                                            style={{ background: 'linear-gradient(135deg, #3f77d2, #00b4d8)', fontSize: '0.85rem' }}>
                                            {group.mouldParts.length} Mould{group.mouldParts.length > 1 ? 's' : ''}
                                        </span>
                                        <button
                                            className="btn btn-sm d-flex align-items-center gap-1"
                                            title={`Export ${group.MasterCode} Moulds`}
                                            onClick={(e) => { e.stopPropagation(); exportMasterCSV(group); }}
                                            style={{
                                                background: 'linear-gradient(135deg, #007bff, #00b4d8)',
                                                color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 600,
                                            }}
                                ...
                                {/* Expanded: Mould Part detail table */}
                                {expanded[group.M_ID] && (
                                    <div className="table-responsive">
                                        <table className="table table-striped table-hover align-middle mb-0">
                                            <thead>
                                            ...
                                            <tbody>
                                                {group.mouldParts.map((m, idx) => (
                                                    <tr key={m.MapID}>
                                                        <td>{idx + 1}</td>
                                                        <td className="fw-semibold">{m.MouldPartCode}</td>
                                                        <td>
                                                            <span className="badge" style={{ background: '#6c5ce7' }}>{m.MouldPart}</span>
                                                        </td>
                                                        <td>{m.ShortFormSize}</td>
...
MouldMapping.propTypes = {
    auth: PropTypes.any.isRequired,
};

export default MouldMapping