...
const UnmapAsset = ({ auth }) => {
  const [loading, setLoading] = useState(false); // Loader state
  const [rowCount, setRowCount] = useState(0);
  const API_URL = getConfig().REACT_APP_API_URL;
  // Table FOR Mapped Assets
  ...
  const StatusRenderer = (params) => {
    return (
      <div>
        <span className='badge bg-danger'>With Out RFID</span>
      </div>
    )
  }
  ...
  // client-side date filter comparator for the "DD-MM-YYYY HH:MM:SS" cell text
  ...
  const columdef = useMemo(() => [
    ...
    {
      headerName: 'Location Code', headerClass: 'agheader', field: 'LocationCode', filter: true, floatingFilter: true, editable: true, valueGetter: (params) => {
        const value = params.data?.LocationCode;
        return value || "-";
      }
    },
    {
      headerName: 'Building', headerClass: 'agheader', field: 'Building', filter: true, floatingFilter: true, editable: true, valueGetter: (params) => {
        const value = params.data?.Building;
        return value || "-";
      }
    },
    {
      headerName: 'Floor', headerClass: 'agheader', field: 'Floor', filter: true, floatingFilter: true, editable: true, valueGetter: (params) => {
        const value = params.data?.Floor;
        return value || "-";
      }
    },
    {
      headerName: 'Room', headerClass: 'agheader', field: 'Room', filter: true, floatingFilter: true, editable: true, valueGetter: (params) => {
        const value = params.data?.Room;
        return value || "-";
      }
    },
    {
      headerName: "Vendor", headerClass: 'agheader', field: "VendorName", filter: true, floatingFilter: true, editable: true, valueGetter: (params) => {
  ...
  const downloadExcel = async () => {
    ...
    const rowData = [];
    (await fetchAllRows()).forEach((d) => rowData.push(d));
    ...
    const sheet = workbook.addWorksheet("WithOut RFID Assets");
    // TURN OFF GRIDLINES
    ...
    titleCell.value = `WITHOUT RFID ASSETS - ${formattedDate}`;
    ...
    saveAs(new Blob([buffer]), "WithOut RFID Assets.xlsx");
  };
  ...
  const handlepdf = async (selectedDate) => {
    ...
    const title = 'WithOut RFID Assets';
    ...
    // ✅ Header columns
    const headers = [
      'S.No', 'Status', 'AllocatedStatus', 'RFID Number', 'Asset ID', 'Asset Type', 'Description', 'Category',
      'SubCategory', 'Department', 'LocationCode', 'Building', 'Floor', 'Room', 'Vendor Name', 'Phone Number', 'Created Date', 'Created By', 'Updated Date', 'Update By'
    ];
    ...
    if (bodyData.length > 0) {
      ...
      doc.save("WithOut RFID Assets.pdf");
    } else {
  ...
  const [serverSide, setServerSide] = useState(null); // null=deciding, false=client, true=server paged
      ...
      getRows: async (p) => {
        ...
        try {
          const { data } = await axios.post(`${API_URL}/EnrolledAssetInfoPaged`, {
            departmentname: auth.departmentname, mode: 'WithOutRFID',
        ...
        } catch (error) { console.error('Error fetching Without RFID page:', error); p.failCallback(); }
      },
    };
  ...
  const initGrid = async () => {
    ...
    try {
      ...
      setRowCount(t);
      if (t > CLIENT_SIDE_LIMIT) { setServerSide(true); }
      else {
        const { data } = await axios.post(`${API_URL}/EnrolledAssetInfo`, {
          departmentname: auth.departmentname, mode: 'WithOutRFID',
          branchid: auth.branchid, BranchAccess: auth.BranchAccess,
        });
        setrowdef(data.send || []); setServerSide(false);
      }
    } catch (error) { console.error('Error initializing Without RFID grid:', error); setServerSide(false); }
  ...
  return (
    <div>
      ...
      <Card className='mt-4'><div className="card-header pro-header p-1">
        <div className="d-flex justify-content-center align-items-center">
          {/* Center Title */}
          <div className="text-center" >
            <h3 className='text-white'> <i class="bi bi-ticket-perforated fs-3 ms-2"></i> Without RFID Assets</h3>
          </div>
        </div>
      </div>
        ...
        <CardBody >
          ...
          <div style={{ height: "500px" }} className='ag-theme-quartz mt-2'>
            {serverSide !== null && (
              <AgGridReact ref={gridRef} columnDefs={columdef} rowSelection={"multiple"}
                {...(serverSide
...
UnmapAsset.propTypes = {
  auth: PropTypes.any.isRequired,
};

export default UnmapAsset
