...
const Maintained = ({ auth }) => {
  ...
  const downloadExcel = async () => {
    const columnDefs = [
      ...
      { header: "Maintained", key: "MaintainbyName" },
      {
    ...
    const sheet = workbook.addWorksheet("Maintained Master");
    // TURN OFF GRIDLINES
    ...
    titleCell.value = `MAINTAINED MASTER - ${formattedDate}`;
    ...
    saveAs(new Blob([buffer]), "Maintained Master.xlsx");
  };


  const [Register, setRegister] = useState({
    Maintained: '', MaintainId: '',
  })
  //pdf
  const handlepdf = async (selectedDate) => {
    ...
    // ✅ Header columns
    const headers = [
      'S.No', 'Maintained', 'Created By', 'Created Date', 'Update By', 'Updated Date',
    ...
    if (bodyData.length > 0) {
      ...
      doc.save("Maintained Master.pdf");
    } else {
  ...
  const handleSave = async (e) => {
    ...
    if (Register.Maintained === '') {
      Swal.fire({
        icon: 'warning',
        title: 'Please Enter Maintained By'
      })
      return
    }
    const isValid = MaintainedData.some((item) => {
      return (
        item.MaintainbyName.toString().trim().toLowerCase() === Register.Maintained.toString().trim().toLowerCase()

      );
    });
    if (isValid) {
      swal({
        title: 'Maintain by Name Already Exists',
        icon: 'error',
      });
      return;
    }
    try {
      ...
      const alldata = { ...Register, mode: 'I', branchid: auth.branchid, Createdby: auth.empid, MaintainId: '' }
      // console.log(alldata)
  ...
  const EditRenderer = (params) => {
    ...
    return <div>
      <button className="border-0 mt-1 rounded-circle d-flex justify-content-center align-items-center"
        ...
        }} onClick={() => handleEditMaintained(params.data.MaintainById)} >
        <FaEdit className="fs-5" />
      </button>
    </div>

  }
  // Handle Edit Maintained
  const handleEditMaintained = async (MaintainById) => {
    ...
    try {
      ...
      modal.show();
      const alldata = { MaintainId: MaintainById, mode: 'SD', branchid: auth.branchid }
      console.log(alldata)
      const response = await axios.post(`${API_URL}/MaintainedConfig`, alldata)
      if (response.status === 200) {
        const { Maintained, MaintainById } = response.data[0];
        setRegister({
          ...Register,
          Maintained: Maintained,
          MaintainId: MaintainById
        })
      }
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        fetchData();
        Swal.fire({
          title: 'Maintained By Edited Successfully',
          icon: 'success',
          confirmButtonText: 'Done'
        })
  ...
  const columndef = [
    ...
    { headerName: "Maintained By", headerClass: 'agheader', field: 'MaintainbyName', filter: true, floatingFilter: true, editable: true },
    {
      headerName: "Created By", filter: true, floatingFilter: true, editable: true, headerClass: 'agheader', field: "Createdby",
      valueGetter: (params) => {
    ...
    {
      headerName: "Created Date", headerClass: 'agheader', field: "Createddate", filter: true, floatingFilter: true, editable: true,
      valueGetter: (params) => {
    ...
    {
      headerName: "Last Modified By", filter: true, floatingFilter: true, editable: true, headerClass: 'agheader', field: "updatedby",
      valueGetter: (params) => {
    ...
    {
      headerName: "Last Modified Date", filter: true, floatingFilter: true, editable: true, headerClass: 'agheader', field: "updateddate",
      valueGetter: (params) => {
    ...
    { headerName: "Edit", headerClass: 'agheader', pinned: right, field: 'Edit', cellRenderer: EditRenderer, width: 100 },
  ...
  const handleUploadData = async () => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        const response = await axios.post(`${API_URL}/MaintainedUploadData`, formData, {
        ...
        if (unuploadedFilePath) {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              link.setAttribute('download', 'unuploaded_Maintained_data.xlsx');
  ...
  const handleAddMaintainedClick = () => {
    if (auth.branchid === "0" || auth.branchid === 0) {
      Swal.fire({
        icon: 'warning',
        title: 'Invalid Selection',
  ...
  return (
    ...
    <div className=''>
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header p-2 pro-header">
              ...
              <button type="button" className="btn-close btn-close-white border border-danger me-2" data-bs-dismiss="modal"></button>
            </div>
      ...
      {/* Modal for Add Maintained */}
      <div className="modal fade" id="exampleModal3" tabIndex="-1" aria-labelledby="exampleModalLabel3">
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header p-2 pro-header">
              <h1 className="modal-title fs-5 text-white" id="exampleModalLabel3">
                Add Maintained By Details
              </h1>
              <button
                type="button"
                className="btn-close btn-close-white border border-danger me-2"
                data-bs-dismiss="modal"
                onClick={() => setRegister({ ...Register, Maintained: '', MaintainId: '' })}
              />
            </div>
            <div className="modal-body">
              <div>
                <label className="form-label">
                  Maintained By <span className="text-danger">*</span>
                </label>
                <input
                  className="form-control"
                  placeholder="Enter Maintained By"
                  value={Register.Maintained}
                  onChange={(e) => setRegister({ ...Register, Maintained: e.target.value })}
                />
              </div>
      ...
      {/* Modal for Edit Maintained */}
      <div className="modal fade" id="exampleModalEDIT" tabIndex="-1" aria-labelledby="exampleModalEditLabel" data-bs-backdrop="static">
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content">
            <div className="modal-header p-2 pro-header">
              <h1 className="modal-title fs-5 text-white" id="exampleModalEditLabel">
                Edit Maintained By
              </h1>
              <button
                type="button"
            ...
            <div className="modal-body">
              ...
              <div className="mt-4 text-center d-flex  justify-content-end">
                <button
                  ...
                  className="btn btn-danger mx-1"
                  onClick={() => {
                    // Optionally refetch original value or just clear
                    setRegister({ ...Register, Maintained: '', MaintainId: '' });
                  }}
                >
                  Clear
                </button>
      ...
      {Uploadvisible && (
        <div className={`modal fade ${Uploadvisible ? 'show' : ''}`} style={{ display: Uploadvisible ? 'block' : 'none' }}>
          <div className="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable" data-bs-backdrop="false">
            <div className="modal-content card" >
              ...
              <div className="modal-body">
                ...
                <div className="download-sample-template text-center">
                  ...
                  <div>
                    <a href="/MaintainedTemp.xlsx" className=' nav-link text-decoration-underline ' download>Click to Download</a>
                  </div>
                </div>
      ...
      <div className=' pb-5 card'>
        ...
        {/* <div className='card-header d-flex justify-content-between p-2 pro-header' >
          <h3 className='text-white'> <GrHostMaintenance className='fs-4 mb-1 me-1' style={{marginLeft:'550px'}} /> Maintained Details </h3>
          <i
          ...
          <div className="d-flex align-items-center">
            ...
            <div className="text-center" style={{ width: "34%" }}>
              <h3 className="text-white m-0  text-nowrap">
                <GrHostMaintenance className='fs-4 mb-1 me-1' /> Maintained By Details
              </h3>
            </div>
          ...
          </CButton>
          {
            permission.EditStatus === null || permission.EditStatus === 'i' ? '' :
              <CButton type="submit" color="success" variant="outline" className='me-2 btn-hover-effect'
                onClick={handleAddMaintainedClick}
              >
        ...
        <div className='ag-theme-quartz mx-5 mt-2' style={{ height: "500px" }}>
          <AgGridReact ref={gridRef} rowData={MaintainedData} columnDefs={columndef} rowSelection={"multiple"} autoGroupColumnDef={autoGroupColumnDef} pagination={pagination} paginationPageSize={paginationPageSize} paginationPageSizeSelector={paginationPageSizeSelector} getRowHeight={() => 55} />
        </div>
