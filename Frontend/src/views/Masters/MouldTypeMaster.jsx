...
//             const alldata = {
//                 mode: 'S',
//                 MouldTypeId: '', MouldTypeCode: '', MouldTypeName: '',
//                 Description: '', Status: '', CreatedBy: ''
//             };
//             const response = await axios.post(`${API_URL}/MouldTypeMaster`, alldata);
//             setRowData(response.data || []);
//         } catch (error) {
//             console.error('Error fetching Mould Type details:', error);
//             Swal.fire({ title: 'Error', text: 'Unable to fetch Mould  Type details', icon: 'error' });
//         } finally {
//             setLoading(false);
//         }
//     };
...
//     const loadMouldType = async (data) => {
//         try {
//             setLoading(true);
//             const alldata = { mode: 'E', PartTypeId: data.MouldTypeId };
//             const response = await axios.post(`${API_URL}/MouldTypeMaster`, alldata);
//             const item = Array.isArray(response.data) ? response.data[0] : response.data;
//             setRegister({
//                MouldTypeId: item?.PartTypeId || data.MouldTypeId || '',
//                MouldTypeCode: item?.PartTypeCode || '',
//                MouldTypeName: item?.PartTypeName || '',
//                 Description: item?.Description || '',
//                 Status: item?.Status || '',
//                 CreatedBy: item?.CreatedBy || ''
//             });
//         } catch (error) {
//             console.error('Error loading Part Type details:', error);
//             Swal.fire({ title: 'Error', text: 'Unable to load Part Type details', icon: 'error' });
//         } finally {
//             setLoading(false);
//         }
//     };

//     const handleView = async (data) => { await loadMouldType(data); setViewModalVisible(true); };
//     const handleEdit = async (data) => { await loadMouldType(data); setEditModalVisible(true); };
...
//         return (
//             <div>
//                 <button className='mt-1 ms-1' onClick={() => handleDelete(params.data.PartTypeId)}
...
//             setLoading(true);
//             const alldata = {
//                 mode: 'D', PartTypeId: Editid,
//                 PartTypeCode: '', PartTypeName: '', Description: '', Status: '', CreatedBy: ''
//             };
//             const response = await axios.post(`${API_URL}/PartTypeMaster`, alldata);
...
//         } catch (err) {
//             console.error(err);
//             Swal.fire({ title: 'Error', text: 'Unable to delete Part Type', icon: 'error' });
//         } finally {
//             setLoading(false);
//         }
//     };
...
//         const fieldsToCheck = [
//             { key: 'PartTypeCode', message: 'Please Enter Part Type Code' },
//             { key: 'PartTypeName', message: 'Please Enter Part Type Name' },
//             { key: 'Status', message: 'Please Select Status' },
//         ];
...
//             (item) => item.PartTypeCode?.trim().toLowerCase() === Register.PartTypeCode?.trim().toLowerCase()
//         );
//         if (isExisting) {
//             Swal.fire({ text: "This Part Type Code Already Exists", icon: "warning" });
//             return;
//         }
...
//             setLoading(true);
//             const alldata = {
//                 PartTypeCode: Register.PartTypeCode.trim(),
//                 PartTypeName: Register.PartTypeName.trim(),
//                 Description: Register.Description.trim(),
//                 Status: Register.Status.trim(),
...
//         } catch (err) {
//             console.error(err);
//             Swal.fire({ title: 'Error', text: 'Unable to save Part Type details', icon: 'error' });
//         } finally {
//             setLoading(false);
//         }
//     };
...
//             setLoading(true);
//             const alldata = {
//                 PartTypeId: Register.PartTypeId,
//                 PartTypeCode: Register.PartTypeCode.trim(),
//                 PartTypeName: Register.PartTypeName.trim(),
//                 Description: Register.Description.trim(),
//                 Status: Register.Status.trim(),
...
//         } catch (err) {
//             console.error(err);
//             Swal.fire({ title: 'Error', text: 'Unable to update Part Type details', icon: 'error' });
//         } finally {
//             setLoading(false);
//         }
//     };
...
//     const columdef = [
//         { headerName: "Part Type Code", field: "PartTypeCode", filter: true, floatingFilter: true, headerClass: 'agheader', width: 180 },
//         { headerName: "Part Type Name", field: "PartTypeName", filter: true, floatingFilter: true, headerClass: 'agheader', width: 200 },
//         { headerName: "Description", field: "Description", filter: true, floatingFilter: true, headerClass: 'agheader', width: 250 },
//         { headerName: "Status", field: "Status", filter: true, floatingFilter: true, headerClass: 'agheader', width: 120 },
...
//         headerCheckboxSelection: true, field: "PartTypeId", flex: 1, minWidth: 240,
...
//                 const columnMapping = [
//                     { header: "Part Type Code", key: "PartTypeCode" },
//                     { header: "Part Type Name", key: "PartTypeName" },
...
//             fileName: 'PartType_Details.csv',
//             columnKeys: ['PartTypeCode', 'PartTypeName', 'Description', 'Status']
//         });
//     };
...
//             title: "Are you sure?", text: "Once uploaded, you will not be able to check the Part Type list immediately!",
...
//             const response = await axios.post(`${API_URL}/PartTypeUploadData`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
...
//                     text: "Some Part Type data could not be uploaded. Please download the file to see the errors.",
//                     icon: 'warning', buttons: { cancel: "OK", download: { text: "Download File", value: "download" } }
//                 }).then((value) => {
//                     if (value === "download") {
//                         const link = document.createElement('a');
//                         link.href = `${API_URL}${unuploadedFilePath}`;
//                         link.setAttribute('download', 'unuploaded_PartType_data.xlsx');
...
//     // =========================================================
//     // FORM FIELDS (reusable for Add / Edit / View)
//     // =========================================================

//     const formFields = (disabled = false) => (
//         <div className="row">
//             <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
//                 <label className="form-label">Part Type Code {!disabled && <span className="text-danger">*</span>}</label>
//                 <input type="text" className="form-control" placeholder="Enter Part Type Code"
//                     value={Register.PartTypeCode} disabled={disabled}
//                     onChange={(e) => setRegister({ ...Register, PartTypeCode: e.target.value })} />
//             </div>
//             <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
//                 <label className="form-label">Part Type Name {!disabled && <span className="text-danger">*</span>}</label>
//                 <input type="text" className="form-control" placeholder="Enter Part Type Name"
//                     value={Register.PartTypeName} disabled={disabled}
//                     onChange={(e) => setRegister({ ...Register, PartTypeName: e.target.value })} />
//             </div>
//             <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
//                 <label className="form-label">Description</label>
//                 <textarea className="form-control" rows="2" placeholder="Enter Description"
//                     value={Register.Description} disabled={disabled}
//                     onChange={(e) => setRegister({ ...Register, Description: e.target.value })} />
//             </div>
//             <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
//                 <label className="form-label">Status {!disabled && <span className="text-danger">*</span>}</label>
//                 <select className="form-control" value={Register.Status} disabled={disabled}
//                     onChange={(e) => setRegister({ ...Register, Status: e.target.value })}>
...
//             <div>
//                 {/* EXPORT MODAL */}
//                 <div className="modal fade" id="exampleModalPartType" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
//                         <CModalTitle className="text-white">Part Type Information</CModalTitle>
//                     </CModalHeader>
...
//                     <CModalHeader style={{ background: '#3f77d2' }}>
//                         <CModalTitle className="text-white">Edit Part Type</CModalTitle>
//                     </CModalHeader>
...
//                     <CModalHeader style={{ background: '#3f77d2' }}>
//                         <CModalTitle className="text-white">Add Part Type</CModalTitle>
//                     </CModalHeader>
//                     <CModalBody>{formFields(false)}</CModalBody>
//                     <CModalFooter>
...
//                                         <a href="/PartTypeMasterTemp.xlsx" className="nav-link text-decoration-underline" download>Click to Download</a>
//                                     </div>
...
//                             <FaCog className="fs-4" /> Part Type Master
//                         </h4>
...
//                         <button className="d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#exampleModalPartType"
...
// MouldTypeMaster.propTypes = {
//     auth: PropTypes.any.isRequired,
// };

// export default MouldTypeMaster;
...
const MouldTypeMaster = ({ auth }) => {
    ...
    const loadMouldType = async (data) => {
        try {
            setLoading(true);
            const alldata = { mode: 'E', MouldTypeId: data.MouldTypeId };
            const response = await axios.post(`${API_URL}/MouldTypeMaster`, alldata);
            const item = Array.isArray(response.data) ? response.data[0] : response.data;
            setRegister({
                MouldTypeId: item?.MouldTypeId || data.MouldTypeId || '',
                MouldTypeCode: item?.MouldTypeCode || '',
                MouldTypeName: item?.MouldTypeName || '',
                Description: item?.Description || '',
                Status: item?.Status || '',
                CreatedBy: item?.CreatedBy || ''
            });
        } catch (error) {
            console.error('Error loading Mould Type details:', error);
            Swal.fire({ title: 'Error', text: 'Unable to load Mould Type details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const DeleteRenderer = (params) => {
        ...
        return (
            <div>
                <button className='mt-1 ms-1' onClick={() => handleDelete(params.data.MouldTypeId)}
                    style={{ background: 'linear-gradient(135deg, #ff4d4d, #ff1a1a)', color: '#fff', border: 'none', borderRadius: '50%', padding: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 12px rgba(255, 0, 0, 0.35)', transition: 'all 0.3s ease' }}
                    onMouseEnter={(e) => { e.currentTarget.style.background = 'linear-gradient(135deg, #ff6666, #ff3333)'; e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 6px 18px rgba(255, 80, 80, 0.45)'; }}
                    onMouseLeave={(e) => { e.currentTarget.style.background = 'linear-gradient(135deg, #ff4d4d, #ff1a1a)'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 4px 12px rgba(255, 0, 0, 0.35)'; }}>
                    <CIcon icon={cilTrash} size="xl" />
    ...
    const handleconfirmDelete = async () => {
        try {
            setLoading(true);
            const alldata = {
                mode: 'D', MouldTypeId: Editid,
                MouldTypeCode: '', MouldTypeName: '', Description: '', Status: '', CreatedBy: ''
            };
            const response = await axios.post(`${API_URL}/MouldTypeMaster`, alldata);
            if (response.status === 200) {
                setDeletevisible(false);
            ...
            console.error(err);
            Swal.fire({ title: 'Error', text: 'Unable to delete Mould Type', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const validateFields = () => {
        const fieldsToCheck = [
            { key: 'MouldTypeCode', message: 'Please Enter Mould Type Code' },
            { key: 'MouldTypeName', message: 'Please Enter Mould Type Name' },
            { key: 'Status', message: 'Please Select Status' },
        ];
        for (const field of fieldsToCheck) {
            if (!Register[field.key]?.trim()) {
    ...
    const handlecheck = async () => {
        ...
        const isExisting = rowData.some(
            (item) => item.MouldTypeCode?.trim().toLowerCase() === Register.MouldTypeCode?.trim().toLowerCase()
        );
        if (isExisting) {
            Swal.fire({ text: "This Mould Type Code Already Exists", icon: "warning" });
            return;
        }
        ...
        try {
            setLoading(true);
            const alldata = {
                MouldTypeCode: Register.MouldTypeCode.trim(),
                MouldTypeName: Register.MouldTypeName.trim(),
                Description: Register.Description.trim(),
                Status: Register.Status.trim(),
                CreatedBy: auth.employeename,
                mode: 'I'
            ...
            console.error(err);
            Swal.fire({ title: 'Error', text: 'Unable to save Mould Type details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handlechange = async () => {
        ...
        try {
            setLoading(true);
            const alldata = {
                MouldTypeId: Register.MouldTypeId,
                MouldTypeCode: Register.MouldTypeCode.trim(),
                MouldTypeName: Register.MouldTypeName.trim(),
                Description: Register.Description.trim(),
                Status: Register.Status.trim(),
            ...
            console.error(err);
            Swal.fire({ title: 'Error', text: 'Unable to update Mould Type details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const columdef = [
        { headerName: "Mould Type Code", field: "MouldTypeCode", filter: true, floatingFilter: true, headerClass: 'agheader', width: 180 },
        { headerName: "Mould Type Name", field: "MouldTypeName", filter: true, floatingFilter: true, headerClass: 'agheader', width: 200 },
        { headerName: "Description", field: "Description", filter: true, floatingFilter: true, headerClass: 'agheader', width: 250 },
        { headerName: "Status", field: "Status", filter: true, floatingFilter: true, headerClass: 'agheader', width: 120 },
        { headerName: "View", field: "View", headerClass: 'agheader', cellRenderer: ViewRenderer, width: 80, pinned: 'right' },
        {
    ...
    const autoGroupColumnDef = useMemo(() => ({
        headerCheckboxSelection: true, field: "MouldTypeId", flex: 1, minWidth: 240,
        cellRendererParams: { checkbox: true }
    }), []);
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            if (filteredData.length > 0) {
                const columnMapping = [
                    { header: "Mould Type Code", key: "MouldTypeCode" },
                    { header: "Mould Type Name", key: "MouldTypeName" },
                    { header: "Description", key: "Description" },
                    { header: "Status", key: "Status" },
                ];
                const columnHeaders = columnMapping.map(col => col.header);
                ...
                doc.save('Mould_Type_Master.pdf');
            } else {
                Swal.fire({ title: 'No Data', text: 'No data available to export', icon: 'warning' });
            }
        } catch (error) {
    ...
    const onExportClick = () => {
        if (!gridRef.current?.api) return;
        gridRef.current.api.exportDataAsCsv({
            fileName: 'MouldType_Details.csv',
            columnKeys: ['MouldTypeCode', 'MouldTypeName', 'Description', 'Status']
        });
    };
    ...
    const handleUploadData = async () => {
        if (!uploadxl) { Swal.fire({ text: 'Please Select Upload File', icon: 'warning' }); return; }
        const result = await Swal.fire({
            title: "Are you sure?", text: "Once uploaded, you will not be able to check the Mould Type list immediately!",
            icon: "warning", showCancelButton: true, cancelButtonText: 'Cancel', confirmButtonText: 'Upload'
        });
        if (!result.isConfirmed) return;
        setLoading(true);
        try {
            ...
            formData.append('CreatedBy', auth.employeename);
            formData.append('branchid', auth.branchid);
            const response = await axios.post(`${API_URL}/MouldTypeUploadData`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
            const { uploadcount, unuploadedFilePath } = response.data;
            if (unuploadedFilePath) {
                await fetchData();
                swal({
                    heightAuto: true, title: `Total Uploaded Count: ${uploadcount}`,
                    text: "Some Mould Type data could not be uploaded. Please download the file to see the errors.",
                    icon: 'warning', buttons: { cancel: "OK", download: { text: "Download File", value: "download" } }
                }).then((value) => {
                    if (value === "download") {
                        const link = document.createElement('a');
                        link.href = `${API_URL}${unuploadedFilePath}`;
                        link.setAttribute('download', 'unuploaded_MouldType_data.xlsx');
                        document.body.appendChild(link); link.click(); link.parentNode.removeChild(link);
                    }
                });
            } else {
    ...
    const formFields = (disabled = false) => (
        <div className="row">
            <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
                <label className="form-label">Mould Type Code {!disabled && <span className="text-danger">*</span>}</label>
                <input type="text" className="form-control" placeholder="Enter Mould Type Code"
                    value={Register.MouldTypeCode} disabled={disabled}
                    onChange={(e) => setRegister({ ...Register, MouldTypeCode: e.target.value })} />
            </div>
            <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
                <label className="form-label">Mould Type Name {!disabled && <span className="text-danger">*</span>}</label>
                <input type="text" className="form-control" placeholder="Enter Mould Type Name"
                    value={Register.MouldTypeName} disabled={disabled}
                    onChange={(e) => setRegister({ ...Register, MouldTypeName: e.target.value })} />
            </div>
            <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
                <label className="form-label">Description</label>
                <textarea className="form-control" rows="2" placeholder="Enter Description"
    ...
    return (
        <>
            ...
            <div>
                {/* EXPORT MODAL */}
                <div className="modal fade" id="exampleModalMouldType" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content">
                            <div className="modal-header p-2" style={{ background: '#3f77d2' }}>
                                <h1 className="modal-title fs-5 text-white" id="exampleModalLabel">Download Format</h1>
                ...
                <CModal size="xl" alignment="center" visible={viewModalVisible} onClose={() => { setViewModalVisible(false); handleClear(); }} backdrop="static">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Mould Type Information</CModalTitle>
                    </CModalHeader>
                    <CModalBody>{formFields(true)}</CModalBody>
                </CModal>
                ...
                <CModal size="xl" alignment="center" visible={editModalVisible} onClose={() => { setEditModalVisible(false); handleClear(); }} backdrop="static">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Edit Mould Type</CModalTitle>
                    </CModalHeader>
                    <CModalBody>{formFields(false)}</CModalBody>
                    <CModalFooter>
                        {modalFooterClose(() => { setEditModalVisible(false); handleClear(); })}
                ...
                <CModal visible={isModalVisible} onClose={() => { setIsModalVisible(false); handleClear(); }} backdrop='static' size="xl" alignment="center">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Add Mould Type</CModalTitle>
                    </CModalHeader>
                    <CModalBody>{formFields(false)}</CModalBody>
                    <CModalFooter>
                        {modalFooterClose(() => { setIsModalVisible(false); handleClear(); })}
                            ...
                            <div className="modal-content card">
                                ...
                                <div className="modal-body">
                                    ...
                                    <div className="download-sample-template text-center">
                                        <p>Important ⚠</p>
                                        <span className='text-danger'>Download The Below The Template That Colum Name Based Enter The Data Then Upload here</span>
                                        <a href="/MouldTypeMasterTemp.xlsx" className="nav-link text-decoration-underline" download>Click to Download</a>
                                    </div>
                                    <div className="text-center mt-3">
                                        <button className="btn btn-danger mx-2 btn-hover-effect" onClick={() => { setUploadvisible(false); setuploadxl(null); }}>CANCEL</button>
                                        <button className="btn btn-success mx-2 btn-hover-effect" onClick={handleUploadData}>Upload Data</button>
                ...
                <div className='card'>
                    <div className='card-header d-flex justify-content-between align-items-center p-3' style={{ background: '#106FB2' }}>
                        <h4 className="mb-0 text-white d-flex align-items-center gap-2">
                            <FaCog className="fs-4" /> Mould Type Master
                        </h4>
                        <button className="btn-close btn-close-white" onClick={() => navigate("/Settings/Configure")}></button>
                    </div>
                    ...
                    <div className="d-flex justify-content-end mt-2 gap-2 px-3">
                        ...
                        <button className="d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#exampleModalMouldType"
                            style={{ background: 'linear-gradient(135deg, #ff4b2b, #ff0000)', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 600, padding: '10px 18px', boxShadow: '0 4px 15px rgba(255, 0, 0, 0.35)', cursor: 'pointer' }}>
                            <i className="bi bi-cloud-download"></i> Export
                        </button>
