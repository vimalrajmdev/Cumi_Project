...
const RackMaster = ({ auth }) => {
    ...
    const initialRegister = { RackId: '', RackCode: '', RackName: '', CreatedBy: '' };
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.post(`${API_URL}/RackMaster`, {
                mode: 'S', RackId: '', RackCode: '', RackName: '', CreatedBy: ''
            });
            setRowData(response.data || []);
        } catch (error) {
            console.error('Error fetching Rack details:', error);
            Swal.fire({ title: 'Error', text: 'Unable to fetch Rack details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const loadRack = async (data) => {
        try {
            setLoading(true);
            const response = await axios.post(`${API_URL}/RackMaster`, { mode: 'E', RackId: data.RackId });
            const item = Array.isArray(response.data) ? response.data[0] : response.data;
            setRegister({
                RackId: item?.RackId || data.RackId || '',
            ...
            Swal.fire({ title: 'Error', text: 'Unable to load Rack details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };

    const handleView = async (data) => { await loadRack(data); setViewModalVisible(true); };
    const handleEdit = async (data) => { await loadRack(data); setEditModalVisible(true); };
    ...
    const DeleteRenderer = (params) => {
        ...
        return (
            <div>
                <button className='mt-1 ms-1' onClick={() => handleDelete(params.data.RackId)}
    ...
    try {
        ...
        console.log('[RackMaster Delete] response:', response.data);

        const result = response.data?.[0];

        // Rack is referenced in LinkLocationMaster
        if (result?.status === 'REFERENCE_EXISTS') {
        ...
        Swal.fire({
            title: 'Delete Failed',
            text: 'Rack could not be deleted.',
            icon: 'error',
            confirmButtonText: 'OK'
        });

    } catch (err) {
        console.error('[RackMaster Delete] error:', err);

        Swal.fire({
            title: 'Error',
            text: err.response?.data?.message || 'Unable to delete Rack',
            icon: 'error',
            confirmButtonText: 'OK'
        });
    } finally {
    ...
    const validateFields = () => {
        const fieldsToCheck = [
            { key: 'RackCode', message: 'Please Enter Rack Code' },
            { key: 'RackName', message: 'Please Enter Rack Name' },
        ];
    ...
    const handlecheck = async () => {
        ...
        const isExisting = rowData.some(
            (item) => item.RackCode?.trim().toLowerCase() === Register.RackCode?.trim().toLowerCase()
        );
        if (isExisting) {
            Swal.fire({ text: "This Rack Code Already Exists", icon: "warning" });
            return;
        }
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/RackMaster`, {
                mode: 'I',
                RackCode: Register.RackCode.trim(),
                RackName: Register.RackName.trim(),
            ...
            console.error(err);
            Swal.fire({ title: 'Error', text: 'Unable to save Rack details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handlechange = async () => {
        ...
        try {
            ...
            console.error(err);
            Swal.fire({ title: 'Error', text: 'Unable to update Rack details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const columdef = [
        { headerName: "Rack Code", field: "RackCode", filter: true, floatingFilter: true, headerClass: 'agheader', width: 180 },
        { headerName: "Rack Name", field: "RackName", filter: true, floatingFilter: true, headerClass: 'agheader', width: 250 },
    ...
    const autoGroupColumnDef = useMemo(() => ({
        headerCheckboxSelection: true, field: "RackId", flex: 1, minWidth: 240,
    ...
    const onExportClick = () => {
        ...
        gridRef.current.api.exportDataAsCsv({
            fileName: 'Rack_Details.csv',
            columnKeys: ['RackCode', 'RackName']
        });
    };
    ...
    const handleUploadData = async () => {
        ...
        const result = await Swal.fire({
            title: "Are you sure?", text: "Once uploaded, you will not be able to check the Rack list immediately!",
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/RackUploadData`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
            ...
            if (unuploadedFilePath) {
                ...
                swal({
                    ...
                    text: "Some Rack data could not be uploaded. Please download the file to see the errors.",
                    icon: 'warning', buttons: { cancel: "OK", download: { text: "Download File", value: "download" } }
                }).then((value) => {
                    if (value === "download") {
                        const link = document.createElement('a');
                        link.href = `${API_URL}${unuploadedFilePath}`;
                        link.setAttribute('download', 'unuploaded_Rack_data.xlsx');
    ...
    const formFields = (disabled = false) => (
        <div className="row">
            <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
                <label className="form-label">Rack Code {!disabled && <span className="text-danger">*</span>}</label>
                <input type="text" className="form-control" placeholder="Enter Rack Code"
                    value={Register.RackCode} disabled={disabled}
                    onChange={(e) => setRegister({ ...Register, RackCode: e.target.value })} />
            </div>
            <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
                <label className="form-label">Rack Name {!disabled && <span className="text-danger">*</span>}</label>
                <input type="text" className="form-control" placeholder="Enter Rack Name"
                    value={Register.RackName} disabled={disabled}
                    onChange={(e) => setRegister({ ...Register, RackName: e.target.value })} />
            </div>
        </div>
    );
    ...
    return (
        <>
            ...
            <div>
                {/* EXPORT MODAL */}
                <div className="modal fade" id="exampleModalRack" tabIndex="-1" aria-hidden="true">
                ...
                <CModal size="xl" alignment="center" visible={viewModalVisible} onClose={() => { setViewModalVisible(false); handleClear(); }} backdrop="static">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Rack Information</CModalTitle>
                    </CModalHeader>
                ...
                <CModal size="xl" alignment="center" visible={editModalVisible} onClose={() => { setEditModalVisible(false); handleClear(); }} backdrop="static">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Edit Rack</CModalTitle>
                    </CModalHeader>
                ...
                <CModal visible={isModalVisible} onClose={() => { setIsModalVisible(false); handleClear(); }} backdrop='static' size="xl" alignment="center">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Add Rack</CModalTitle>
                    </CModalHeader>
                    <CModalBody>{formFields(false)}</CModalBody>
                    <CModalFooter>
                            ...
                            <div className="modal-content card">
                                ...
                                <div className="modal-body">
                                    ...
                                    <div className="download-sample-template text-center">
                                        ...
                                        <a href="/RackMasterTemp.xlsx" className="nav-link text-decoration-underline" download>Click to Download</a>
                                    </div>
                ...
                <div className='card'>
                    <div className='card-header d-flex justify-content-between align-items-center p-3' style={{ background: '#106FB2' }}>
                        <h4 className="mb-0 text-white d-flex align-items-center gap-2">
                            <FaCog className="fs-4" /> Rack Master
                        </h4>
                    ...
                    <div className="d-flex justify-content-end mt-2 gap-2 px-3">
                        ...
                        <button className="d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#exampleModalRack"
...
RackMaster.propTypes = { auth: PropTypes.any.isRequired };
