...
const RowMaster = ({ auth }) => {
    ...
    const initialRegister = { RowId: '', RowCode: '', RowName: '', CreatedBy: '' };
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.post(`${API_URL}/RowMaster`, {
                mode: 'S', RowId: '', RowCode: '', RowName: '', CreatedBy: ''
            });
            setRowData(response.data || []);
        } catch (error) {
            console.error('Error fetching Row details:', error);
            Swal.fire({ title: 'Error', text: 'Unable to fetch Row details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const loadRow = async (data) => {
        try {
            setLoading(true);
            const response = await axios.post(`${API_URL}/RowMaster`, { mode: 'E', RowId: data.RowId });
            const item = Array.isArray(response.data) ? response.data[0] : response.data;
            setRegister({
                RowId:     item?.RowId     || data.RowId || '',
            ...
            Swal.fire({ title: 'Error', text: 'Unable to load Row details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };

    const handleView = async (data) => { await loadRow(data); setViewModalVisible(true); };
    const handleEdit = async (data) => { await loadRow(data); setEditModalVisible(true); };
    ...
    const DeleteRenderer = (params) => {
        ...
        return (
            <div>
                <button className='mt-1 ms-1' onClick={() => handleDelete(params.data.RowId)}
    ...
    const handleconfirmDelete = async () => {
        try {
            ...
            const response = await axios.post(`${API_URL}/Row`, {
                mode: 'D', RowId: Editid, RowCode: '', RowName: '', CreatedBy: ''
            });
            ...
            console.error(err);
            Swal.fire({ title: 'Error', text: 'Unable to delete Row', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const validateFields = () => {
        const fieldsToCheck = [
            { key: 'RowCode', message: 'Please Enter Row Code' },
            { key: 'RowName', message: 'Please Enter Row Name' },
        ];
    ...
    const handlecheck = async () => {
        ...
        const isExisting = rowData.some(
            (item) => item.RowCode?.trim().toLowerCase() === Register.RowCode?.trim().toLowerCase()
        );
        if (isExisting) {
            Swal.fire({ text: "This Row Code Already Exists", icon: "warning" });
            return;
        }
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/RowMaster`, {
                mode: 'I',
                RowCode:   Register.RowCode.trim(),
                RowName:   Register.RowName.trim(),
            ...
            console.error(err);
            Swal.fire({ title: 'Error', text: 'Unable to save Row details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handlechange = async () => {
        ...
        try {
            ...
            console.error(err);
            Swal.fire({ title: 'Error', text: 'Unable to update Row details', icon: 'error' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const columdef = [
        { headerName: "Row Code", field: "RowCode", filter: true, floatingFilter: true, headerClass: 'agheader', width: 180 },
        { headerName: "Row Name", field: "RowName", filter: true, floatingFilter: true, headerClass: 'agheader', width: 250 },
    ...
    const autoGroupColumnDef = useMemo(() => ({
        headerCheckboxSelection: true, field: "RowId", flex: 1, minWidth: 240,
    ...
    const generatePDF = async () => {
        ...
        try {
            ...
            if (filteredData.length > 0) {
                const columnMapping = [
                    { header: "Row Code", key: "RowCode" },
                    { header: "Row Name", key: "RowName" },
                ];
                doc.autoTable({
    ...
    const onExportClick = () => {
        ...
        gridRef.current.api.exportDataAsCsv({
            fileName: 'Row_Details.csv',
            columnKeys: ['RowCode', 'RowName']
        });
    };
    ...
    const handleUploadData = async () => {
        ...
        const result = await Swal.fire({
            title: "Are you sure?", text: "Once uploaded, you will not be able to check the Row list immediately!",
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/RowUploadData`, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
            ...
            if (unuploadedFilePath) {
                ...
                swal({
                    ...
                    text: "Some Row data could not be uploaded. Please download the file to see the errors.",
                    icon: 'warning', buttons: { cancel: "OK", download: { text: "Download File", value: "download" } }
                }).then((value) => {
                    if (value === "download") {
                        const link = document.createElement('a');
                        link.href = `${API_URL}${unuploadedFilePath}`;
                        link.setAttribute('download', 'unuploaded_Row_data.xlsx');
    ...
    const formFields = (disabled = false) => (
        <div className="row">
            <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
                <label className="form-label">Row Code {!disabled && <span className="text-danger">*</span>}</label>
                <input type="text" className="form-control" placeholder="Enter Row Code"
                    value={Register.RowCode} disabled={disabled}
                    onChange={(e) => setRegister({ ...Register, RowCode: e.target.value })} />
            </div>
            <div className="col-lg-6 col-md-6 col-sm-12 mt-3">
                <label className="form-label">Row Name {!disabled && <span className="text-danger">*</span>}</label>
                <input type="text" className="form-control" placeholder="Enter Row Name"
                    value={Register.RowName} disabled={disabled}
                    onChange={(e) => setRegister({ ...Register, RowName: e.target.value })} />
            </div>
        </div>
    );
    ...
    return (
        <>
            ...
            <div>
                {/* EXPORT MODAL */}
                <div className="modal fade" id="exampleModalRow" tabIndex="-1" aria-hidden="true">
                ...
                <CModal size="xl" alignment="center" visible={viewModalVisible} onClose={() => { setViewModalVisible(false); handleClear(); }} backdrop="static">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Row Information</CModalTitle>
                    </CModalHeader>
                ...
                <CModal size="xl" alignment="center" visible={editModalVisible} onClose={() => { setEditModalVisible(false); handleClear(); }} backdrop="static">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Edit Row</CModalTitle>
                    </CModalHeader>
                ...
                <CModal visible={isModalVisible} onClose={() => { setIsModalVisible(false); handleClear(); }} backdrop='static' size="xl" alignment="center">
                    <CModalHeader style={{ background: '#3f77d2' }}>
                        <CModalTitle className="text-white">Add Row</CModalTitle>
                    </CModalHeader>
                    <CModalBody>{formFields(false)}</CModalBody>
                    <CModalFooter>
                            ...
                            <div className="modal-content card">
                                ...
                                <div className="modal-body">
                                    ...
                                    <div className="download-sample-template text-center">
                                        ...
                                        <a href="/RowMasterTemp.xlsx" className="nav-link text-decoration-underline" download>Click to Download</a>
                                    </div>
                ...
                <div className='card'>
                    <div className='card-header d-flex justify-content-between align-items-center p-3' style={{ background: '#106FB2' }}>
                        <h4 className="mb-0 text-white d-flex align-items-center gap-2">
                            <FaCog className="fs-4" /> Row Master
                        </h4>
                    ...
                    <div className="d-flex justify-content-end mt-2 gap-2 px-3">
                        ...
                        <button className="d-flex align-items-center gap-2" data-bs-toggle="modal" data-bs-target="#exampleModalRow"
...
RowMaster.propTypes = { auth: PropTypes.any.isRequired };
