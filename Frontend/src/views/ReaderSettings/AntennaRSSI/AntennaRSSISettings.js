...
import { BsFileEarmarkPdfFill, BsRssFill, BsTrash } from 'react-icons/bs';
...
const AntennaRSSISettings = ({ auth }) => {
    ...
    const colDefs = [
        { field: 'Id', headerClass: 'agheader', headerName: 'ID', width: 200 },
        { field: 'ReaderIP', headerClass: 'agheader', headerName: 'Reader IP', width: 250 },
        { field: 'AntennaID', headerClass: 'agheader', headerName: 'Antenna ID', width: 250 },
        { field: 'MinRSSI', headerClass: 'agheader', headerName: 'MinRSSI', width: 250 },
        {
            field: "edit", headerClass: 'agheader', headerClass: 'agheader', headerName: "Edit", width: 250,
            cellRenderer: (params) => {
        ...
        {
            field: "delete", headerClass: 'agheader', headerClass: 'agheader', headerName: "Delete", width: 250,
            cellRenderer: (params) => {
    ...
    const handleedit = async (Id) => {
        try {
            const res = await axios.post(`${API_URL}/AntennaRSSI`, {
                Id: Number(Id),
                ReaderIP: '',
                AntennaID: 0,
                MinRSSI: 0,
    ...
    const handleupdate = async () => {
        try {
            const payload = {
                ...
                ReaderIP: Antenna.ReaderIP,
                AntennaID: Number(Antenna.AntennaID),
                MinRSSI: Number(Antenna.MinRSSI),
                createdby: 1,
                updateby: 1,
                branchid: 0,
                mode: 'U',
            ...
            await axios.post(`${API_URL}/AntennaRSSI`, payload);
    ...
    const handledelete = async (params) => {
        const Id = params
        const alldata = { ...Antenna, Id: Id, branchid: '', mode: 'D' }
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.isConfirmed) {
                const response = await axios.post(`${API_URL}/AntennaRSSI`, alldata)
    ...
    const handlesave = async () => {
        ...
        if (Antenna.MinRSSI == '') {
            swal({
                text: "please Enter MinRSSI",
                icon: "warning",
            })
            return
        }

        const alldata = { ...Antenna, mode: 'I', branchid: '' }
        const response = await axios.post(`${API_URL}/AntennaRSSI`, alldata)
        if (response.status === 200) {
            Swal.fire({
                position: "center",
                icon: "success",
                title: "Your Antenna RSSIS Setting has been saved",
                showConfirmButton: false,
                timer: 1500
            });
            Antennafetch();
    ...
    const Antennafetch = async () => {
        const alldata = { ...Antenna, mode: 'F', branchid: '' }
        const response = await axios.post(`${API_URL}/AntennaRSSI`, alldata)
        console.log('res.data', response.data)
        setRowData(response.data.locations)
    }
    useEffect(() => {
        Antennafetch();
    }, [])
    ...
    const onExportClick = async () => {
        ...
        const sheet = workbook.addWorksheet("Antenna RSSI Settings");
        ...
        titleCell.value = `Antenna RSSI Settings - ${formattedDate}`;
        ...
        saveAs(new Blob([buffer]), "Antenna RSSI Settings.xlsx");
    };
    ...
    const handlepdf = async (selectedDate) => {
        ...
        const title = 'Antenna RSSI Settings';
        ...
        if (bodyData.length > 0) {
            ...
            doc.save("Antenna RSSI Settings.pdf");
        } else {
            ...
            <Card className='mt-4'>
                <CardHeader className='pro-header'>
                    <div className='d-flex justify-content-center text-white'>
                        <BsRssFill style={{ fontSize: '30px' }} className='mb-2 me-1' /><h3 className='text-white'>Antenna RSSI Settings</h3>
                    </div>
                </CardHeader>
                <CardBody>
                    ...
                    >
                        ...
                        <CModalBody>
                            <CRow>
                                ...
                                <CCol>
                                    <CFormLabel>MinRSSI <span className='text-danger'>*</span></CFormLabel>
                                    <CFormInput type='text' placeholder='Enter MinRSSI' onChange={(e) => setAntenna({ ...Antenna, MinRSSI: e.target.value })} />
                                </CCol>
                            </CRow>
                    ...
                    >
                        ...
                        <CModalBody>
                            <CRow>
                                <CCol>
                                    <CFormLabel>Reader IP <span className='text-danger'>*</span></CFormLabel>
                                    <CFormInput type='text' placeholder='Enter Reader IP' onChange={(e) => setAntenna({ ...Antenna, ReaderIP: e.target.value })} value={Antenna.ReaderIP} />
                                </CCol>
                                <CCol>
                                    <CFormLabel>Antenna ID <span className='text-danger'>*</span></CFormLabel>
                                ...
                                <CCol>
                                    <CFormLabel>MinRSSI <span className='text-danger'>*</span></CFormLabel>
                                    <CFormInput type='text' placeholder='Enter MinRSSI' onChange={(e) => setAntenna({ ...Antenna, MinRSSI: e.target.value })} value={Antenna.MinRSSI} />
                                </CCol>
                            </CRow>
...
AntennaRSSISettings.propTypes = {
    auth: PropTypes.any.isRequired,
};
export default AntennaRSSISettings
