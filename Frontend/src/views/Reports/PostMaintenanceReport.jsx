...
const PostMaintenanceReport = ({ auth }) => {
  ...
  const downloadExcel = async () => {
    const columnDefs = [
      ...
      {
        header: "Maintenance Invoice", key: "MaintenanceInvoiceNumber",

      },
      {
        header: "MaintenanceCost", key: "MaintenanceCost",
      },
      {
        header: "Maintenance RegDate", key: "MaintenanceRegDate",
      }, {
        header: "Maintenance Done RegDate", key: "MaintenanceDoneRegDate",
      },
      {
        header: "Activity", key: "Activity",
      },
      {
        header: "Inspected By", key: "InspectedBy",
      }, {
    ...
    const sheet = workbook.addWorksheet("Maintenance Done Report");
    // TURN OFF GRIDLINES
    ...
    titleCell.value = `MAINTENANCE DONE REPORT - ${formattedDate}`;
    ...
    rowData.forEach((row, index) => {
      const rowValues = columnDefs.map((c) => {
        ...
        if (c.key === "MaintenanceRegDate" || c.key === "MaintenanceDoneRegDate") {
          return formatDate(v);
        }
    ...
    saveAs(new Blob([buffer]), "Maintenance Done Report.xlsx");
  };
  ...
  //   const title = 'Maintenance Done Report';
  ...
  //       { header: "Maintenance Reg Date", key: "MaintenanceRegDate" },
  //       { header: "Post Maintenance Reg Date", key: "MaintenanceDoneRegDate" },
  //       { header: "Activity", key: "Activity" },
  ...
  //     doc.save(`Maintenance_Done_Report.pdf`);
  //   } else {
  ...
  const handlepdf = async (selectedDate) => {
    ...
    // ✅ Header columns
    const headers = [
      'S.No', 'RFID Number', 'Asset ID', 'Asset Name', 'Vendors', 'Maintenance Invoice', 'Maintenance Cost', 'Maintenance RegDate', 'Maintenance Done RegDate', 'Activity', 'Location', 'Inspected By', 'Description'
    ...
    const bodyData = filteredData.map((item, index) => [
      ...
      item.AssetName || "-",
      item.Vendors || "-",
      item.MaintenanceInvoiceNumber || "-",
      item.MaintenanceCost || "-",
      formatDate(item.MaintenanceRegDate) || "-",
      formatDate(item.MaintenanceDoneRegDate) || "-",
      item.Activity || "-",
      item.Branch || "-",
    ...
    if (bodyData.length > 0) {
      ...
      doc.save("Maintenance Done Report.pdf");
    } else {
  ...
  const columndef = [
    ...
    { headerCheckboxSelection: true, checkboxSelection: true, filter: true, floatingFilter: true, width: 50 },
    {
      headerName: "RFID Number", headerClass: 'agheader', field: "RFIDnumber", filter: true, floatingFilter: true
      , valueGetter: (params) => {
    ...
    },
    {
      ...
      , valueGetter: (params) => {
        const value = params.data?.AssetName;
        return value || "-";
      }
    },
    {
      headerName: "Vendors", headerClass: 'agheader', field: "Vendors", filter: true, floatingFilter: true
      , valueGetter: (params) => {
        const value = params.data?.Vendors;
        return value || "-";
      }
    },
    {
      headerName: "Maintenance Invoice", headerClass: 'agheader', field: "MaintenanceInvoiceNumber", filter: true, floatingFilter: true
      , valueGetter: (params) => {
        const value = params.data?.MaintenanceInvoiceNumber;
        return value || "-";
      }
    },
    {
      headerName: "Maintenance Cost", headerClass: 'agheader', field: "MaintenanceCost", filter: true, floatingFilter: true
      , valueGetter: (params) => {
        const value = params.data?.MaintenanceCost;
        return value || "-";
      }
    },
    {
      headerName: "Maintenance Reg Date", headerClass: 'agheader', field: "MaintenanceRegDate", filter: true, floatingFilter: true
      , valueGetter: (params) => {
        const date = params.data.MaintenanceRegDate;
        if (!date) return '-';
    ...
    {
      headerName: "Post Maintenance Reg Date", headerClass: 'agheader', field: "MaintenanceDoneRegDate", filter: true, floatingFilter: true
      , valueGetter: (params) => {
        const date = params.data.MaintenanceDoneRegDate;
        if (!date) return '-';
        const d = new Date(date);
        const yyyy = d.getUTCFullYear();
        const mm = String(d.getUTCMonth() + 1).padStart(2, '0');
    ...
    {
      headerName: "Activity", field: "Activity", headerClass: 'agheader', filter: true, floatingFilter: true
      , valueGetter: (params) => {
        const value = params.data?.Activity;
        return value || "-";
      }
    },
    {
      headerName: "Inspected By", headerClass: 'agheader', field: "InspectedBy", filter: true, floatingFilter: true
      , valueGetter: (params) => {
        const value = params.data?.InspectedBy;
        return value || "-";
      }
    },
    {
      headerName: "Description", headerClass: 'agheader', field: "Description", filter: true, floatingFilter: true
      , valueGetter: (params) => {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const alldata = { mode: 'PostMaintenanceReport', departmentname: auth.departmentname, branchid: auth.branchid, BranchAccess: auth.BranchAccess }
      const response = await axios.post(`${API_URL}/FetchPostMaintenanceDetails`, alldata);
      ...
      switch (selectedOption) {
        ...
        case 'Oneday': {
          ...
          filteredData = rowdef.filter(item => {
            const itemDate = new Date(item.MaintenanceDoneRegDate);
  ...
  return (
    <>
      ...
      <div className=''>
        ...
        <Card className="mx-auto mt-2" id='box-shadow' style={{ maxWidth: "700px" }}>
          <CardHeader className="p-1 pro-header">
            <div className='d-flex justify-content-center mt-1'>
              ...
              <h3 className="text-white">Maintenance Done Report</h3>
            </div>
          </CardHeader>
        ...
        <div className='card  mt-1' id='box-shadow'>
...
export default PostMaintenanceReport
