...
const AllReports = ({ auth }) => {
    ...
    const downloadExcel = async () => {
        const columnDefs = [
            ...
            { header: "Purchase Date", key: "PDate" },
        ...
        const sheet = workbook.addWorksheet("All Asset Report");
        // TURN OFF GRIDLINES
        ...
        titleCell.value = `All Asset Report - ${formattedDate}`;
        ...
        saveAs(new Blob([buffer]), "All Asset Report.xlsx");
    };
    // Table for All Asset
    ...
    const handlepdf = async (selectedDate) => {
        ...
        // ✅ Header columns
        const headers = [
            'RFID Number', 'Asset Id', 'Asset Name', 'Asset Type', 'Location Code', 'Description', 'Category', 'Sub Category', 'Model', 'Brand', 'Cost', 'Purchase Date', 'Branch', 'Created By', 'Created Date', 'Last Modified By', 'Last Modified Date'
        ];
    ...
    const fetchdata = async () => {
        try {
            setLoading(true)
            const alldata = { departmentname: auth.departmentname, mode: 'AllAssetReport', branchid: auth.branchid, BranchAccess: auth.BranchAccess }
    ...
    const columndef = [
        ...
        { headerName: "Location Code", headerClass: 'agheader', field: "LocationCode", filter: true, floatingFilter: true 
            ,valueGetter: (params) => {
        ...
        { headerName: "Model", headerClass: 'agheader', field: "Model", filter: true, floatingFilter: true 
            ,valueGetter: (params) => {
                const value = params.data?.Model;
                return value || "-";
            }
        },
        { headerName: "Brand", headerClass: 'agheader', field: "Brand", filter: true, floatingFilter: true 
            ,valueGetter: (params) => {
                const value = params.data?.Brand;
                return value || "-";
            }
        },
        { headerName: "Cost", headerClass: 'agheader', field: "PCost", filter: true, floatingFilter: true 
            ,valueGetter: (params) => {
                const value = params.data?.PCost;
                return value || "-";
            }
        },
        {
            headerName: "Purchase Date", headerClass: 'agheader', field: "PDate",
            filter: true, floatingFilter: true,
            valueGetter: (params) => {
                const date = params.data.PDate;
                if (!date) return '-';
        ...
        { headerName: "Branch", headerClass: 'agheader', field: "Branch", filter: true, floatingFilter: true },
        {
    ...
    //             case 'Oneday': {
    //                 filteredData = rowdef.filter(item => new Date(item.Createddate).toDateString() === new Date().toDateString());
    //                 break;
    //             }
    //             case 'week': {
    //                 const Oneday = new Date();
    //                 // Calculate the start of the week (Sunday)
    //                 const weekStart = new Date(Oneday);
    //                 weekStart.setDate(Oneday.getDate() - Oneday.getDay()); // Set to the previous Sunday
    //                 weekStart.setHours(0, 0, 0, 0); // Normalize to start of the day

    //                 // Calculate the end of the week (Saturday)
    //                 const weekEnd = new Date(Oneday);
    //                 weekEnd.setDate(Oneday.getDate() + (6 - Oneday.getDay())); // Set to the next Saturday
    ...
    //                     start.setHours(0, 0, 0, 0);  // full day start

    //                     const end = new Date(toDate);
    //                     end.setHours(23, 59, 59, 999); // full day end
    ...
    return (
        <>
            ...
            {/* <h3 className=" text-center m-0">All Asset Report</h3> */}
            ...
            <Card className="mx-auto mt-2" id='box-shadow' style={{ maxWidth: "700px" }}>
                <CardHeader className="p-1 pro-header">
                    <div className='d-flex justify-content-center mt-1'>
                        ...
                        <h3 className="text-white ">All Asset Report</h3>
                    </div>
                </CardHeader>
            ...
            <div className='mt-3 card' id='box-shadow'>
                <div style={{ height: "460px" }} className='ag-theme-quartz'>
