...
const ExternalTransferReport = ({ auth }) => {
    ...
    const downloadExcel = async () => {
        const columnDefs = [
            ...
            {
                header: "Transferred BranchName", key: "TransferredBranchName",
            },
            {
            ...
            {
                header: "Paymen tStatus", key: "PaymentStatus",
            },
            { header: "Asset Cost", key: "AssetCost" },
            { header: "Received Amount", key: "ReceivedAmount" },
            { header: "Transferred By", key: "Createdby" },
            { header: "Transferred Date", key: "CreatedDate" },

        ];
        const rowData = gridRef.current.api.getRenderedNodes().map(n => n.data);
        const workbook = new ExcelJS.Workbook();
        const sheet = workbook.addWorksheet("External Transfer Report");
        // TURN OFF GRIDLINES
        ...
        titleCell.value = `EXTERNAL TRANSFER REPORT - ${formattedDate}`;
        ...
        saveAs(new Blob([buffer]), "External Transfer Report.xlsx");
    };
    ...
    const handlepdf = async (selectedDate) => {
        ...
        const title = 'External Transfer Report';
        ...
        // ✅ Header columns
        const headers = [
            'S.No', 'RFID Number', 'Asset ID', 'Transferred BranchName', 'Received BranchName', 'Remarks', 'Paymen tStatus', 'Asset Cost', 'Received Amount', 'Transferred By', 'Transferred Date'
        ...
        if (bodyData.length > 0) {
            autoTable(doc, {
                ...
                didDrawPage: (data) => {
                    ...
                    const footerText = `Printed By: ${auth.Createdby} | Printed On: ${new Date().toLocaleDateString()} | Time: ${new Date().toLocaleTimeString()}`;
            ...
            doc.save("External Transfer Report.pdf");
        } else {
    ...
    const fetchdata = async () => {
        ...
        try {
            const alldata = { departmentname: auth.departmentname, mode: 'S', branchid: auth.branchid, BranchAccess: auth.BranchAccess, Transferredbranchid: '', Receivedbranchid: '', TransferedBy: '' }
    ...
    const columndef = [
        ...
        { headerName: "Transferred BranchName", headerClass: 'agheader', field: "TransferredBranchName", filter: true, floatingFilter: true },
        ...
        { headerName: "Paymen tStatus", headerClass: 'agheader', headerClass: 'agheader', field: "PaymentStatus", filter: true, floatingFilter: true },
        { headerName: "Asset Cost", headerClass: 'agheader', field: "AssetCost", filter: true, floatingFilter: true },
        { headerName: "Received Amount", headerClass: 'agheader', field: "ReceivedAmount", filter: true, floatingFilter: true },
        { headerName: "Transferred By", headerClass: 'agheader', field: "Createdby", filter: true, floatingFilter: true },

        {
            headerName: "Transferred Date", headerClass: 'agheader', field: "CreatedDate",
            valueGetter: (params) => {
    ...
    return (
        <div className=''>
            ...
            {/* <h3>External Transfer Report</h3> */}
            <Card className="mx-auto mt-2" id='box-shadow' style={{ maxWidth: "700px" }}>
                <CardHeader className="p-2 pro-header">
                    <h3 className="text-white text-center m-0">External Transfer Report</h3>
                </CardHeader>
...
ExternalTransferReport.propTypes = {
    auth: PropTypes.any.isRequired,
};
export default ExternalTransferReport
